                ...
                @if(count($integrations) === 0)
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                        <div class="text-center py-12">
                            ...
                            <h3 class="mt-2 text-sm font-medium text-gray-900">{{ __('No Integrations Installed') }}</h3>
                        ...
                        <div class="flex -mb-px">
                            @foreach($integrations as $slug => $integration)
                                <a href="{{ route('settings.integrations', ['tab' => $slug]) }}" 
                                   class="px-6 py-3 text-sm font-medium border-b-2 {{ $activeTab === $slug ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
                                    {{ $integration['name'] }}
                                </a>
                            @endforeach
                        </div>
