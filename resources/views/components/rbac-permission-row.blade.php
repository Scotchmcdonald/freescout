...
<tr class="border-b border-gray-100 dark:border-gray-700/50 last:border-b-0 hover:bg-gray-50/50 dark:hover:bg-gray-700/30 transition-colors duration-100"
    x-show="open && matchesSearch('{{ $perm->name }}')"
    x-transition>
    <td class="px-4 py-2.5 text-sm pl-10">
        <div class="flex flex-col">
            <span class="text-gray-700 dark:text-gray-300 font-medium">{{ $perm->label ?? $perm->name }}</span>
            <span class="text-xs text-gray-400 dark:text-gray-500 font-mono">{{ $perm->name }}</span>
        </div>
    </td>
    @foreach($roles as $role)
        <td class="px-3 py-2.5 text-center">
            @if($role->is_super_admin)
                ...
                <span class="inline-flex items-center justify-center w-5 h-5 rounded text-green-500 dark:text-green-400"
            ...
            @else
                <label class="relative inline-flex items-center cursor-pointer">
                    ...
                    <div class="w-5 h-5 rounded border-2 border-gray-300 dark:border-gray-500
