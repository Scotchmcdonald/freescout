...
<th class="px-3 py-3 text-center min-w-[120px] max-w-[160px]">
    <div class="flex flex-col items-center gap-1">
        <span class="text-sm font-semibold text-neutral-700 dark:text-neutral-200 truncate max-w-full"
              title="{{ $role->label ?? $role->name }}">
            {{ $role->label ?? $role->name }}
        </span>

        @if($role->is_super_admin)
            <span class="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide bg-warning-100 dark:bg-warning-900/30 text-warning-700 dark:text-warning-400">
        ...
        @else
            <span class="text-[10px] text-neutral-400 dark:text-neutral-500 font-mono">{{ $role->scope }}</span>
        @endif
