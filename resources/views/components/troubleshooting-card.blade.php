...
<div class="rounded-md bg-{{ $color }}-50 p-4 border border-{{ $color }}-200 shadow-sm my-4">
    <div class="flex">
        ...
        <div class="ml-3 flex-1 md:flex md:justify-between">
            <div class="text-sm text-{{ $color }}-700">
                ...
                @if($code)
                    <div class="mt-2 bg-white p-2 rounded border border-{{ $color }}-200 font-mono text-xs select-all cursor-pointer" onclick="navigator.clipboard.writeText(this.innerText); alert('Copied!')">
                        {{ $code }}
                    </div>
                @endif
            </div>
            @if($actionText)
                <p class="mt-3 text-sm md:mt-0 md:ml-6">
                    <a href="{{ $actionUrl }}" class="whitespace-nowrap font-medium text-{{ $color }}-700 hover:text-{{ $color }}-600 flex items-center">
                        {{ $actionText }}
