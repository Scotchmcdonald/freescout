        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2">
                    <x-customer-profile-tabs :customer="$customer" />
                                ...
                                @foreach($conversations as $conversation)
                                    <div class="border border-neutral-200 rounded-lg p-4 hover:bg-neutral-50 transition">
                                        <div class="flex items-start justify-between">
                                            <div class="flex-1 min-w-0">
                                                <div class="flex items-center space-x-2 mb-2">
                                                    <a href="{{ route('conversations.show', $conversation) }}" 
                                                       class="text-base font-medium text-neutral-900 hover:text-primary-600 truncate">
                                                        {{ $conversation->subject }}
                                                    </a>
                                                    @if($conversation->status == 1)
                                                        <span class="flex-shrink-0 px-2 py-0.5 text-xs font-medium bg-success-100 text-success-800 rounded">
                                                            {{ __('Active') }}
                                                        </span>
                                                    @elseif($conversation->status == 2)
                                                        <span class="flex-shrink-0 px-2 py-0.5 text-xs font-medium bg-neutral-100 text-neutral-800 rounded">
                                                            {{ __('Closed') }}
                                                        </span>
                                                    @elseif($conversation->status == 3)
                                                        <span class="flex-shrink-0 px-2 py-0.5 text-xs font-medium bg-warning-100 text-warning-800 rounded">
                                                            {{ __('Pending') }}
                                                        </span>
                                                    @endif
                                                </div>
                                            ...
                                            <div class="text-right text-sm text-neutral-500 ml-4 flex-shrink-0">
