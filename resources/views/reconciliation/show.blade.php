...
@section('title', 'Reconciliation Run Details')
    ...
    <div class="mb-8">
        ...
        <div class="flex items-start justify-between">
            ...
            <div class="flex items-center space-x-2">
                @php $statusInfo = $run->getStatusInfo(); @endphp
                <span class="px-3 py-1 text-sm font-medium rounded-full bg-{{ $statusInfo['color'] }}-100 text-{{ $statusInfo['color'] }}-800">
                    {{ $statusInfo['status'] }}
                </span>
                        ...
                        @foreach($discrepanciesByType as $type => $discrepancies)
                            ...
                            @foreach($discrepancies as $discrepancy)
                                <tr class="hover:bg-neutral-50 transition-colors duration-150">
                                    ...
                                    <td class="px-6 py-4">
                                        @php $severityInfo = $discrepancy->getSeverityInfo(); @endphp
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-{{ $severityInfo['color'] }}-100 text-{{ $severityInfo['color'] }}-800">
                                            <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                                <path fill-rule="evenodd" d="{{ $severityInfo['icon'] }}" clip-rule="evenodd"/>
                                            </svg>
                                            {{ $severityInfo['label'] }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4">
                                        @php $resolutionInfo = $discrepancy->getResolutionInfo(); @endphp
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-{{ $resolutionInfo['color'] }}-100 text-{{ $resolutionInfo['color'] }}-800">
                                    ...
                                    <td class="px-6 py-4 text-right text-sm font-medium">
                                        @if(!$discrepancy->isResolved())
                                            <button 
                                                @click="openResolveModal({{ json_encode([
                                                    'id' => $discrepancy->id,
    ...
    <div x-show="showResolveModal" 
        ...
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div x-show="showResolveModal"
                ...
                <form method="POST" 
                      :action="`/admin/reconciliation/discrepancies/${selectedDiscrepancy?.id}/resolve`"
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left flex-1">
                                ...
                                <div class="mt-4">
                                    ...
                                    >
                                        ...
                                        <option value="updated_local">Updated Local Record</option>
                                        <option value="ignored_expected">Expected Difference - No Action</option>
                                        <option value="escalated">Escalated to Support</option>
                                        <option value="data_migration">Data Migration Issue</option>
