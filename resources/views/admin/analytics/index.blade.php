    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Revenue forecasting, growth trends, and business insights</p>
            </div>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div class="bg-white rounded-lg shadow p-6 hover:shadow-md transition-shadow duration-200" style="border-left: 4px solid var(--theme-primary-500, #6366f1)">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-xs uppercase tracking-wide font-medium text-gray-500">Monthly Recurring Revenue</p>
                            <p class="text-3xl font-bold text-gray-900 mt-2">${{ number_format($metrics['mrr'], 0) }}</p>
                ...
                <div class="bg-white rounded-lg shadow p-6 hover:shadow-md transition-shadow duration-200" style="border-left: 4px solid var(--theme-success-500, #10b981)">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-xs uppercase tracking-wide font-medium text-gray-500">Active Clients</p>
                            <p class="text-3xl font-bold text-gray-900 mt-2">{{ $metrics['active_clients'] }}</p>
                            <p class="text-sm text-gray-500 mt-1">+{{ $metrics['new_clients_this_month'] }} this month</p>
                        </div>
                ...
                <div class="bg-white rounded-lg shadow p-6 hover:shadow-md transition-shadow duration-200" style="border-left: 4px solid var(--theme-info-500, #3b82f6)">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-xs uppercase tracking-wide font-medium text-gray-500">Avg Revenue/Client</p>
                            <p class="text-3xl font-bold text-gray-900 mt-2">${{ number_format($metrics['arpc'], 0) }}</p>
                            <p class="text-sm text-gray-500 mt-1">All-time average</p>
                        </div>
                ...
                <div class="bg-white rounded-lg shadow p-6 hover:shadow-md transition-shadow duration-200" style="border-left: 4px solid var(--theme-warning-500, #f59e0b)">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-xs uppercase tracking-wide font-medium text-gray-500">Unbilled Services</p>
                            <p class="text-3xl font-bold text-gray-900 mt-2">${{ number_format($metrics['unbilled_value'], 0) }}</p>
                            <p class="text-sm text-gray-500 mt-1">Approved, pending</p>
                        </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {{-- Revenue Trends --}}
                <div class="bg-white rounded-lg shadow p-6">
                    <h3 class="text-lg font-semibold text-gray-900 mb-4">Revenue Trends (12 Months)</h3>
                            ...
                            @foreach(array_slice($revenueTrends, -6) as $trend)
                                <div>
                                    <div class="flex justify-between items-center mb-1">
                                        ...
                                        <span class="text-sm font-bold text-gray-900">${{ number_format($trend['revenue'], 0) }}</span>
                                    </div>
                                    ...
                                    <p class="text-xs text-gray-500 mt-0.5">{{ $trend['invoice_count'] }} invoices</p>
                                </div>
                            @endforeach
                        </div>
                    @else
                ...
                {{-- Revenue Forecast --}}
                <div class="bg-white rounded-lg shadow p-6">
                    <h3 class="text-lg font-semibold text-gray-900 mb-4">Revenue Forecast (6 Months)</h3>
                    ...
                    @if(!empty($forecasts))
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead>
                                    <tr>
                                        <th scope="col" class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Month</th>
                                        <th scope="col" class="px-3 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Forecast</th>
                                        <th scope="col" class="px-3 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Confidence</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-gray-200">
                                    @foreach($forecasts as $forecast)
                                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                                            <td class="px-3 py-3 whitespace-nowrap text-sm text-gray-900">{{ $forecast['month'] }}</td>
                    ...
                    @else
                        <div class="text-center py-8">
                            ...
                            <p class="mt-2 text-sm text-gray-500">Insufficient data for forecasting</p>
                            <p class="text-xs text-gray-400 mt-1">Need at least 3 months of revenue history</p>
                        </div>
                    @endif
                </div>
            </div>
