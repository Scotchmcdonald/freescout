...
<div class="editor-wrapper {{ $editorClass }}" x-data="richTextEditor{{ $editorId }}()">
    {{-- Toolbar --}}
    @if ($showToolbar)
    <div class="editor-toolbar border border-neutral-300 rounded-t-md bg-neutral-50 px-2 py-1 flex flex-wrap gap-1 items-center">
        {{-- Text formatting --}}
        <div class="flex gap-1 border-r border-neutral-300 pr-2">
            ...
            <button type="button" @click="editor?.chain().focus().toggleUnderline().run()"
                    :class="{'bg-neutral-200': editor?.isActive('underline')}"
                    class="p-1.5 rounded hover:bg-neutral-200 transition-colors" 
        ...
        <div class="flex gap-1 border-r border-neutral-300 pr-2">
            <button type="button" @click="editor?.chain().focus().toggleHeading({ level: 2 }).run()"
                    ...
                    class="p-1.5 rounded hover:bg-neutral-200 transition-colors text-sm font-semibold" 
                    title="Heading 2">
                H2
            </button>
        ...
        <div class="flex gap-1 border-r border-neutral-300 pr-2">
            <button type="button" @click="setLink()"
                    :class="{'bg-neutral-200': editor?.isActive('link')}"
                    class="p-1.5 rounded hover:bg-neutral-200 transition-colors" 
                    title="Insert Link">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="flex gap-1 border-r border-neutral-300 pr-2">
            ...
            <button type="button" @click="editor?.chain().focus().toggleCode().run()"
                    :class="{'bg-neutral-200': editor?.isActive('code')}"
                    class="p-1.5 rounded hover:bg-neutral-200 transition-colors" 
                    title="Inline Code">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div 
        x-ref="editorElement"
        class="editor-content prose prose-sm max-w-none border @if($showToolbar) border-t-0 @endif border-neutral-300 rounded-b-md p-3 focus-within:ring-2 focus-within:ring-primary-500 focus-within:border-primary-500 bg-white"
