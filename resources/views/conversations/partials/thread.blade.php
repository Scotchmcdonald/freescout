...
<div class="thread border border-gray-200 rounded-lg p-4 mb-4 {{ $is_note ? 'bg-yellow-50' : 'bg-white' }} {{ $is_draft ? 'border-dashed border-orange-400' : '' }}"
    ...
    <div class="flex items-start gap-4">
        ...
        <div class="flex-shrink-0">
            @if ($thread_person)
                <div
                    class="w-10 h-10 rounded-full {{ $is_customer ? 'bg-gray-400' : 'bg-blue-600' }} flex items-center justify-center text-white font-semibold">
        ...
        <div class="flex-1 min-w-0">
            ...
            @if ($is_draft)
                <div class="flex gap-2 mb-3">
                    <button
                        class="px-3 py-1 text-sm bg-blue-600 text-white rounded hover:bg-blue-700 edit-draft-trigger">
                        {{ __('Edit') }}
                    </button>
                    <button
                        class="px-3 py-1 text-sm bg-gray-200 text-gray-700 rounded hover:bg-gray-300 discard-draft-trigger">
                        {{ __('Discard') }}
                    </button>
                </div>
            @endif
