...
<div class="thread border border-neutral-200 rounded-lg p-4 mb-4 {{ $is_note ? 'bg-warning-50' : 'bg-white' }} {{ $is_draft ? 'border-dashed border-warning-400' : '' }}"
    id="thread-{{ $thread->id }}" data-thread-id="{{ $thread->id }}">
    ...
    <div class="flex items-start gap-4">
        ...
        <div class="flex-shrink-0">
            @if ($thread_person)
                <div
                    class="w-10 h-10 rounded-full {{ $is_customer ? 'bg-neutral-400' : 'bg-primary-600' }} flex items-center justify-center text-white font-semibold">
            ...
            @else
                <div
                    class="w-10 h-10 rounded-full bg-neutral-300 flex items-center justify-center text-white font-semibold">
                    ?
                </div>
            @endif
        </div>
        ...
        <div class="flex-1 min-w-0">
            ...
            <div class="flex items-start justify-between mb-3">
                <div class="flex-1">
                    <div class="flex items-center gap-2 mb-1">
                        <span class="font-semibold text-neutral-900">
                            @if ($thread_person)
                                @if ($is_customer && !empty($thread_person->url))
                                    <a href="{{ $thread_person->url() }}" class="text-primary-600 hover:underline">
                        ...
                        @if ($is_note)
                            <span class="px-2 py-0.5 text-xs font-medium bg-warning-200 text-warning-800 rounded">
                                {{ __('Note') }}
                            </span>
                        @endif
                    ...
                    <div class="text-sm text-neutral-500">
                        <a href="#thread-{{ $thread->id }}" class="hover:text-neutral-700"
                    ...
                    @if (!$is_note && ($thread->to || $thread->cc || $thread->bcc))
                        <div class="text-xs text-neutral-600 mt-2 space-y-1">
                            @if ($thread->to)
                                <div>
                ...
                @if (!$is_draft)
                    <div class="flex-shrink-0" x-data="{ open: false }">
                        <button @click="open = !open" class="text-neutral-400 hover:text-neutral-600 p-1 rounded"
                            type="button">
                        ...
                        <div x-show="open" @click.away="open = false"
                            ...
                            <div class="py-1">
                                ...
                                @can('delete', $conversation)
                                    <a href="#" class="block px-4 py-2 text-sm text-danger-600 hover:bg-neutral-100">
                                        {{ __('Delete') }}
                                    </a>
                                @endcan
                            </div>
            ...
            @if ($is_draft)
                <div class="flex gap-2 mb-3">
                    <button
                        class="px-3 py-1 text-sm bg-primary-600 text-white rounded hover:bg-primary-700 edit-draft-trigger">
                        {{ __('Edit') }}
                    </button>
                    <button
                        class="px-3 py-1 text-sm bg-neutral-200 text-neutral-700 rounded hover:bg-neutral-300 discard-draft-trigger">
                        {{ __('Discard') }}
                    </button>
                </div>
            @endif
            ...
            {{-- Thread Body --}}
            <div class="prose max-w-none text-neutral-900 thread-body">
                @php
                    $threadBody = (string) ($thread->body ?? '');
                    $containsHtml = $threadBody !== strip_tags($threadBody);
                    $renderedBody = $containsHtml ? clean($threadBody, 'default') : nl2br(e($threadBody));
                @endphp
                {!! $renderedBody !!}
            </div>
