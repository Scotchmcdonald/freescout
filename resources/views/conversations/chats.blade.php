        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="grid grid-cols-1 md:grid-cols-3 h-[calc(100vh-200px)]">
                    {{-- Chat List Sidebar --}}
                    <div class="md:col-span-1 border-r border-neutral-200 overflow-y-auto">
                        ...
                        <div class="divide-y divide-gray-200">
                            @forelse($conversations ?? [] as $conversation)
                                <a href="{{ route('conversations.chats', ['id' => $conversation->id]) }}" 
                                   class="block p-4 hover:bg-neutral-50 transition"
                                   style="{{ isset($active_conversation) && $active_conversation->id == $conversation->id ? 'background-color: var(--theme-primary-50);' : '' }}">
                                    <div class="flex items-start gap-3">
                                        <div class="w-10 h-10 rounded-full bg-neutral-400 flex items-center justify-center text-white font-semibold flex-shrink-0">
                    ...
                    <div class="md:col-span-2 flex flex-col">
                        @if(isset($active_conversation))
                            ...
                            <div class="p-4 border-b border-neutral-200 bg-white">
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-10 rounded-full bg-neutral-400 flex items-center justify-center text-white font-semibold">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-sm text-neutral-500">{{ $active_conversation->customer_email }}</p>
                                        </div>
                                    </div>
                                    <button type="button" class="text-neutral-400 hover:text-neutral-600">
                            ...
                            {{-- Messages --}}
                            <div class="flex-1 overflow-y-auto p-4 space-y-4 bg-neutral-50">
                                @foreach($active_conversation->threads as $thread)
                                    <div class="flex {{ $thread->type == 1 ? 'justify-start' : 'justify-end' }}">
                                        <div class="max-w-xs lg:max-w-md">
                                            <div class="rounded-lg px-4 py-2 {{ $thread->type == 1 ? 'bg-white border border-neutral-200' : 'bg-primary-600 text-white' }}">
                                                <p class="text-sm">{{ $thread->body }}</p>
                                            </div>
                                            <div class="text-xs text-neutral-500 mt-1 {{ $thread->type == 1 ? 'text-left' : 'text-right' }}">
                                ...
                                <form method="POST" action="{{ route('conversations.reply', $active_conversation) }}">
                                    ...
                                    <div class="flex gap-2">
                                        <input type="text" 
                                               ...
                                               class="flex-1 border-neutral-300 rounded-full shadow-sm focus:border-primary-500 focus:ring-primary-500"
                                               required>
                                        <button type="submit" 
                                                class="px-6 py-2 bg-primary-600 text-white rounded-full hover:bg-primary-700 transition">
                                            {{ __('Send') }}
                                        </button>
                                    </div>
                                </form>
