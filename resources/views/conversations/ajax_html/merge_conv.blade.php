    ...
    <form method="POST" 
        ...
        <div class="space-y-6">
            ...
            @if(isset($conversation))
                <div class="bg-neutral-50 rounded-lg p-4">
                    <div class="text-sm text-neutral-600 mb-2">{{ __('Merging Conversation') }}</div>
                ...
                <div class="space-y-2">
                    <label class="flex items-start">
                        <input type="checkbox" 
                               ...
                               value="1" 
                               checked
                               class="mt-1 rounded border-neutral-300 focus:ring-0"
        ...
        <div class="mt-6 flex items-center justify-end gap-3">
            ...
            <button type="submit" 
                    id="merge-submit-btn"
                    disabled
                    class="px-4 py-2 text-sm font-medium text-white bg-danger-600 rounded-md hover:bg-danger-700 transition disabled:opacity-50 disabled:cursor-not-allowed">
...
<script>
    ...
    function displayMergeResults(conversations) {
        ...
        if (conversations.length === 0) {
            resultsContainer.innerHTML = '<div class="text-sm text-neutral-500 p-4 text-center">{{ __('No conversations found') }}</div>';
            ...
            <div class="border border-neutral-200 rounded-lg p-4 hover:bg-neutral-50 transition">
                <label class="flex items-start gap-3 cursor-pointer">
                    <input type="radio" 
                           ...
                           class="mt-1 rounded-full border-neutral-300 text-primary-600 focus:ring-primary-500">
                    <div class="flex-1">
                        <div class="font-medium text-neutral-900">#${conv.number} - ${conv.subject}</div>
                        <div class="text-sm text-neutral-500 mt-1">${conv.customer_name} (${conv.customer_email})</div>
                    </div>
                </label>
            </div>
        `).join('');
