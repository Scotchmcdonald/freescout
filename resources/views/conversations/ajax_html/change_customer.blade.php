        ...
        <div class="space-y-6">
            ...
            @if(isset($conversation) && $conversation->customer)
                <div class="bg-neutral-50 rounded-lg p-4">
                    <div class="text-sm text-neutral-600 mb-1">{{ __('Current Customer') }}</div>
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <div class="font-medium text-neutral-900">{{ $conversation->customer->getFullName(true) }}</div>
                            <div class="text-sm text-neutral-500">{{ $conversation->customer_email }}</div>
                        </div>
                    </div>
                </div>
            @endif
            ...
            <div class="relative">
                ...
                <div class="relative flex justify-center text-sm">
                    <span class="px-2 bg-white text-neutral-500">{{ __('OR') }}</span>
                </div>
            </div>
            ...
            <div>
                ...
                <div x-show="creating" 
                     class="mt-4 space-y-4 p-4 bg-neutral-50 rounded-lg"
                     style="display: none;">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-neutral-700 mb-1">
                            {{ __('Email Address') }} <span class="text-danger-600">*</span>
                        </label>
                        <input type="email" 
            ...
            <div id="validation-message" class="hidden">
                <div class="bg-warning-50 border-l-4 border-warning-400 p-4">
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <p class="text-sm text-warning-700" id="validation-message-text"></p>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    function displaySearchResults(customers) {
        ...
        if (customers.length === 0) {
            resultsContainer.innerHTML = '<div class="text-sm text-neutral-500 p-2">{{ __('No customers found') }}</div>';
        ...
        resultsContainer.innerHTML = customers.map(customer => `
            <button type="button" 
                    ...
                    class="w-full text-left p-3 border border-neutral-200 rounded hover:bg-neutral-50 transition">
                <div class="font-medium text-neutral-900">${customer.full_name}</div>
                <div class="text-sm text-neutral-500">${customer.email}</div>
            </button>
        `).join('');
