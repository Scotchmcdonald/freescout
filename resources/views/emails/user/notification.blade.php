...
<body bgcolor="#f8f9f9" style="-webkit-text-size-adjust:none; margin: 0;">
	<table bgcolor="#f8f9f9" cellspacing="0" border="0" cellpadding="0" width="100%" id="fsNotifReplyAbove" class="fsNotifReplyAbove" data-fs="fsNotifReplyAbove">
		<tr>
			<td>
				<table class="content" width="100%" border="0" cellspacing="0" cellpadding="0">
				    <tr>
				        <td height="45" valign="bottom"><p align="center" style="font-family:Arial, 'Helvetica Neue', Helvetica, Tahoma, sans-serif; font-size:12px; color:#B5B9BD; line-height:16px; margin:0;">-- Please reply above this line --</p></td>
				    </tr>
				    <tr>
				        <td height="12"></td>
				    </tr>
							    ...
							    <tr>
							        <td colspan="2">
							            <p style="font-family:Arial, 'Helvetica Neue', Helvetica, Tahoma, sans-serif; line-height:15px; margin:0; font-size:12px; color:#a1a6ab; padding-bottom: 0.75em;">
							            	...
							            	@else
								            	@if ($thread->action_type == \App\Models\Thread::ACTION_TYPE_STATUS_CHANGED)
			                                    ...
			                                    @elseif ($thread->action_type == \App\Models\Thread::ACTION_TYPE_USER_CHANGED)
				                                    <strong>{{ $thread->getCreatedBy()->getFullName(true) }}</strong>  
													{{ __("assigned to :person conversation", ['person' => $thread->getAssigneeName(false, $user)]) }}
			                                    @elseif ($thread->type == \App\Models\Thread::TYPE_NOTE)
													...
													<tr>
														<td valign="top">
															<div style="disdivlay:inline; font-family:Arial, 'Helvetica Neue', Helvetica, Tahoma, sans-serif; color:#b5b9bd; font-size:12px; line-height:16px; margin:0;">
																{!! $thread->getActionText('', true, false, $user, '<strong>'.$thread->getCreatedBy()->getFullName(true).'</strong>') !!}
															</div>
														</td>
														<td valign="top">
															<div style="font-family:Arial, 'Helvetica Neue', Helvetica, Tahoma, sans-serif; color:#b5b9bd; font-size:12px; line-height:16px; margin:0;" align="right">{{ \App\Models\User::dateFormat($thread->created_at, 'M j, H:i', $user) }}</div>
														</td>
													</tr>
												</table>
											</td>
							...
							<tr>
								<td>
									<table width="100%" border="0" cellspacing="0" cellpadding="0" style="border-bottom:1px solid #dde3e7;">
									    <tr>
									        <td style="padding: 2em;" bgcolor="@if ($thread->type == \App\Models\Thread::TYPE_MESSAGE){{ config('app.colors')['bg_user_reply'] }}@elseif ($thread->type == \App\Models\Thread::TYPE_NOTE){{ config('app.colors')['bg_note'] }}@else{{ 'ffffff' }}@endif">
									            <table width="100%" border="0" cellspacing="0" cellpadding="0">
									                <tr>
									                    ...
									                    <td valign="top">
									                        <div style="font-family:Arial, 'Helvetica Neue', Helvetica, Tahoma, sans-serif; color:#B5B9BD; font-size:12px; line-height:18px; margin:0;" align="right">{{ \App\Models\User::dateFormat($thread->created_at, 'M j, H:i', $user) }}</div>
									                    </td>
									                </tr>
									                <tr>
		...
		<tr>
			<td height="0" style="font-size: 0px; line-height: 0px; color:#ffffff;">	                    	
				{{-- Message marker for reply detection --}}
				<div style="font-size: 0px; line-height: 0px; color:#ffffff !important;">{{ isset($headers['Message-ID']) ? $headers['Message-ID'] : '' }}</div>
			</td>
		</tr>
	</table>
