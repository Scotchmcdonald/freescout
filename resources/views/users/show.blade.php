                ...
                <div class="lg:col-span-1">
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                        ...
                        <div class="flex items-center mb-6">
                            <div class="h-20 w-20 rounded-full bg-primary-600 flex items-center justify-center text-white text-2xl font-semibold">
                        ...
                        <div class="space-y-4 text-sm">
                            <div>
                                <div class="text-neutral-500 mb-1">{{ __('Role') }}</div>
                                <div class="font-medium">
                                    @if($user->role == 2)
                                        <span class="px-2 py-1 text-xs font-medium bg-primary-100 text-primary-800 rounded">Administrator</span>
                                    @elseif($user->role == 3)
                            ...
                            <div>
                                <div class="text-neutral-500 mb-1">{{ __('Status') }}</div>
                                <div class="font-medium">
                                    @if($user->status == 1)
                            ...
                            @if($user->timezone)
                                <div>
                                    <div class="text-neutral-500 mb-1">{{ __('Timezone') }}</div>
                                    <div class="font-medium">{{ $user->timezone }}</div>
                                </div>
                            @endif
                            
                            <div>
                                <div class="text-neutral-500 mb-1">{{ __('Member Since') }}</div>
                    ...
                    <div class="mt-6 bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                        ...
                        @if($user->mailboxes->isEmpty())
                            <p class="text-sm text-neutral-500">No mailboxes assigned</p>
                        @else
                            <div class="space-y-2">
                                @foreach($user->mailboxes as $mailbox)
                                    <a href="{{ route('mailboxes.view', $mailbox) }}" 
                                       class="block p-3 border border-neutral-200 rounded-lg hover:bg-neutral-50 transition">
                                        <div class="font-medium text-neutral-900">{{ $mailbox->name }}</div>
                                        <div class="text-sm text-neutral-600">{{ $mailbox->email }}</div>
                                    </a>
                                @endforeach
                            </div>
                        @endif
                                            ...
                                            <div class="flex-1">
                                                <div class="flex items-center space-x-2 mb-2">
                                                    ...
                                                    @if($conversation->status == 1)
                                                        <span class="px-2 py-0.5 text-xs font-medium bg-success-100 text-success-800 rounded">Active</span>
                                                    @elseif($conversation->status == 2)
                                                        <span class="px-2 py-0.5 text-xs font-medium bg-neutral-100 text-neutral-800 rounded">Closed</span>
                                                    @endif
                                                </div>
