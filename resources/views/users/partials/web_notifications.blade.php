    ...
    <ul class="divide-y divide-gray-200">
        ...
        @foreach($notifications as $notification)
            ...
            @if($notificationDate !== $lastDate)
                <li class="px-4 py-2 bg-neutral-50 text-xs font-semibold text-neutral-600 uppercase tracking-wider">
                    @if($isToday)
                        {{ __('Today') }}
                    @else
                        {{ $notificationDate }}
            ...
            {{-- Notification item --}}
            <li class="@if(!$notification->read_at) @endif hover:bg-neutral-50 transition"
                   ...
                   class="block px-4 py-3">
                    <div class="flex items-start space-x-3">
                        ...
                        <div class="flex-shrink-0">
                            ...
                            @else
                                <div class="h-10 w-10 rounded-full bg-neutral-400 flex items-center justify-center text-white text-sm font-semibold">
...
@else
    ...
    <div class="text-center py-12 px-4">
        ...
        <h3 class="mt-2 text-sm font-medium text-neutral-900">{{ __('No notifications') }}</h3>
        <p class="mt-1 text-sm text-neutral-500">{{ __("You're all caught up!") }}</p>
    </div>
@endif
