...
<div x-data="{
    ...
    async addRule() {
        ...
        try {
            ...
            const data = await res.json();
            if (data.success) { this.interceptRules = data.rules; this.newRule = ''; }
        } finally { this.submitting = false; }
    },
    ...
    <div x-show="interceptActive" class="mb-6 rounded-lg bg-warning-50 border border-warning-200 p-4">
        <div class="flex items-center">
            <svg class="h-5 w-5 text-warning-500 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/>
            </svg>
            <p class="ml-3 text-sm font-medium text-warning-800">
                Interception active — matched events are <strong>halted</strong> and held. Listeners will NOT fire until you release them.
            </p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div class="lg:col-span-1 space-y-4">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-4">
                ...
                <div class="mb-4">
                    <label for="new-intercept-rule" class="block text-xs font-medium text-neutral-600 mb-1">Add Event Pattern</label>
                    <div class="flex space-x-2">
                        <input x-model="newRule" type="text" id="new-intercept-rule"
                               list="available-intercept-events"
                        ...
                        <button @click="addRule()" :disabled="submitting" class="px-3 py-2 bg-primary-600 text-white text-xs rounded-md hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-primary-500 disabled:opacity-50">
                            Add
                        </button>
                    </div>
                ...
                <div class="space-y-2">
                    <template x-for="(rule, idx) in interceptRules" :key="idx">
                        <div class="flex items-center justify-between py-1.5 px-2 bg-warning-50 rounded text-xs">
                            ...
                            <button @click="removeRule(rule)" class="text-danger-500 hover:text-danger-700 flex-shrink-0" aria-label="Remove rule">
                    ...
                    <p x-show="interceptRules.length === 0" class="text-xs text-neutral-500 italic">No rules configured.</p>
                </div>
            </div>
                    ...
                    <table class="min-w-full divide-y divide-neutral-200">
                        <thead class="bg-neutral-50">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left text-xs font-medium text-neutral-500 uppercase tracking-wider">Event</th>
                                <th class="px-4 py-3 text-left text-xs font-medium text-neutral-500 uppercase tracking-wider">Class</th>
                                <th class="px-4 py-3 text-left text-xs font-medium text-neutral-500 uppercase tracking-wider">Intercepted</th>
                            ...
                            @forelse($pending as $intercept)
                                <tr class="hover:bg-primary-50 transition-colors group" data-id="{{ $intercept->id }}"
                                    ...
                                    <td class="px-2 py-3 text-center" @click.stop>
                                        <input type="checkbox"
                                               :checked="isSelected({{ $intercept->id }})"
                                               @change="toggleSelect({{ $intercept->id }})"
                                               class="rounded border-neutral-300 text-primary-600 focus:ring-primary-500">
                                    </td>
                                    <td class="px-2 py-3 cursor-grab text-neutral-400 hover:text-neutral-600" @click.stop>
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap">
                                        <span class="text-sm font-medium text-neutral-900">{{ $intercept->event_name }}</span>
                                    </td>
                                    <td class="px-4 py-3 whitespace-nowrap">
                                        <code class="text-xs font-mono text-neutral-500">{{ Str::limit($intercept->event_class, 40) }}</code>
                                    </td>
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap text-right space-x-2" @click.stop>
                                        <button @click="fireEvent({{ $intercept->id }})" class="text-success-600 hover:text-success-800 text-xs font-medium" title="Fire this event">
                                            Fire
                                        </button>
                                        <button @click="discardEvent({{ $intercept->id }})" class="text-danger-600 hover:text-danger-800 text-xs font-medium" title="Discard this event">
                                            Discard
                                        </button>
                                    </td>
                                </tr>
                    ...
                    <table class="min-w-full divide-y divide-neutral-200">
                        <thead class="bg-neutral-50">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left text-xs font-medium text-neutral-500 uppercase tracking-wider">Intercepted</th>
                                <th class="px-4 py-3 text-left text-xs font-medium text-neutral-500 uppercase tracking-wider">Resolved</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-neutral-200">
                            @foreach($history as $item)
                                <tr class="hover:bg-neutral-50">
                                    <td class="px-4 py-3 whitespace-nowrap text-sm text-neutral-700">{{ $item->event_name }}</td>
                                    <td class="px-4 py-3 whitespace-nowrap">
                                        @include('middleman::components.status-badge', ['status' => $item->status])
                                    </td>
                                    <td class="px-4 py-3 whitespace-nowrap text-sm text-neutral-500">{{ $item->intercepted_at->diffForHumans() }}</td>
                                    <td class="px-4 py-3 whitespace-nowrap text-sm text-neutral-500">{{ $item->fired_at?->diffForHumans() ?? '-' }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
    ...
    <template x-if="selectedEvent">
        <div x-show="selectedEvent"
            ...
            <div class="sticky top-0 z-10 bg-white border-b border-neutral-200 px-6 py-4 flex items-center justify-between">
                <h3 class="text-lg font-semibold text-neutral-900" x-text="selectedEvent?.event_name || 'Intercept Detail'"></h3>
                <button @click="selectedEvent = null" class="text-neutral-400 hover:text-neutral-500 focus:outline-none focus:ring-2 focus:ring-primary-500 rounded" aria-label="Close panel">
            ...
            <div class="px-6 py-4 space-y-6">
                ...
                <div>
                    <label class="block text-xs font-medium text-neutral-500 uppercase tracking-wide mb-1">Status</label>
                    <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium"
                          :class="selectedEvent?.status === 'pending' ? 'bg-warning-100 text-warning-800' : selectedEvent?.status === 'fired' ? 'bg-success-100 text-success-800' : 'bg-neutral-100 text-neutral-600'"
                          x-text="selectedEvent?.status"></span>
                </div>
                <div>
                    <label class="block text-xs font-medium text-neutral-500 uppercase tracking-wide mb-1">Intercepted At</label>
                ...
                <div>
                    <div class="flex items-center justify-between mb-1">
                        <label class="block text-xs font-medium text-neutral-500 uppercase tracking-wide">Payload</label>
                        <template x-if="selectedEvent?.status === 'pending' && editingPayload !== selectedEvent?.id">
                            <button @click="startEditPayload(selectedEvent)" class="text-xs text-primary-600 hover:text-primary-800 font-medium">
                                Edit
                            </button>
                        </template>
                    </div>
                    ...
                    <template x-if="editingPayload === selectedEvent?.id">
                        <div class="space-y-2">
                            ...
                            <div class="flex space-x-2">
                                <button @click="savePayload(selectedEvent.id)" class="px-3 py-1.5 bg-success-600 text-white text-xs rounded-md hover:bg-success-700">Save</button>
                                <button @click="editingPayload = null" class="px-3 py-1.5 bg-neutral-200 text-neutral-700 text-xs rounded-md hover:bg-neutral-300">Cancel</button>
                            </div>
                        </div>
                    </template>
                    <template x-if="editingPayload !== selectedEvent?.id">
                ...
                <template x-if="selectedEvent?.status === 'pending'">
                    <div class="rounded-lg bg-warning-50 border border-warning-200 p-3">
                        <p class="text-xs text-warning-700">
                            <strong>Caution:</strong> Payload data may become stale if the underlying models have been modified since interception.
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    tbody.querySelectorAll('tr[data-id]').forEach(row => {
        ...
        row.addEventListener('dragend', () => {
            row.style.opacity = '1';
            draggedRow = null;
            tbody.querySelectorAll('tr').forEach(r => r.classList.remove('border-t-2', 'border-primary-500'));
        });
