...
<div x-data="{
    ...
    async fireBatch() {
        ...
        try {
            ...
            this.result = await res.json();
        } catch (e) { this.result = { error: 'Invalid JSON: ' + e.message }; }
        finally { this.submitting = false; }
    },
    getInputType(type) {
        switch (type) {
            case 'int': case 'float': case 'integer': return 'number';
    ...
    <div class="mb-6 rounded-lg bg-info-50 border border-info-200 p-4">
        <div class="flex items-start">
            <svg class="h-5 w-5 text-info-500 flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
            </svg>
            <p class="ml-3 text-sm text-info-700">
                Create and dispatch events manually. Select an event type, fill in the constructor parameters, and fire immediately or hold in the intercept queue.
            </p>
        </div>
    </div>
    ...
    <div x-show="selectedEventClass" x-transition class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 mb-6">
        ...
        <div x-show="tab === 'batch'" class="space-y-4">
            <div>
                ...
                <p class="mt-1 text-xs text-neutral-500">Each object in the array maps to the event constructor parameters.</p>
            </div>
        </div>
    </div>
    ...
    <div x-show="selectedEventClass" x-transition class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 mb-6">
        ...
        <div class="flex items-center space-x-4 mb-4">
            <label class="inline-flex items-center">
                <input type="checkbox" x-model="holdInQueue" class="rounded border-neutral-300 text-warning-600 focus:ring-warning-500">
