...
# Fails if any PHP file in app/, Modules/, or tests/ is missing declare(strict_types=1);
# ==============================================================================
...
MISSING=$(find "$ROOT_DIR/app" "$ROOT_DIR/Modules" "$ROOT_DIR/tests" \
    -type f \
    -name "*.php" \
    ! -name "*.blade.php" \
    ! -path "*/vendor/*" \
    ! -exec grep -q "declare(strict_types=1);" {} \; \
    -print 2>/dev/null)
