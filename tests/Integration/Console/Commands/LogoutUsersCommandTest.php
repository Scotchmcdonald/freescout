...
class LogoutUsersCommandTest extends TestCase
{
    ...
    {
        /** @var LogoutUsers $command */
        $command = $this->app->make(LogoutUsers::class);

        $this->assertSame('freescout:logout-users', $command->getName());
        $this->assertSame('Logout all users', $command->getDescription());
    }
}
